...
public actor HerdrService {
    ...
    /// way `FleetStore.isSSHAuthenticationFailure` already matches OpenSSH's wording.
    ...
    public func disconnect() async -> Bool {
        rpc = nil
        let stopped = await tunnel?.tearDown() ?? true
    ...
    public func gitBranch(at path: String) async -> String? {
        ...
        case .tailcat:
            // No shell on the far side of a tailcat tunnel — the socket is all
            // it carries, so a Space's branch stays unknown.
            output = nil
        }
        return output?
