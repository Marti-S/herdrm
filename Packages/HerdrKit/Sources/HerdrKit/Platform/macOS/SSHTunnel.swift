...
public actor SSHTunnel {
    ...
    /// Stable 64-bit prefix, shared by API sockets and OpenSSH control sockets.
    static func targetIdentifier(for target: String) -> String {
        SHA256.hash(data: Data(target.utf8)).prefix(8)
    ...
    public static func localSocketPath(for target: String) -> String {
        ...
        return dir.appendingPathComponent("\(targetIdentifier(for: target)).sock").path
    }
    ...
    public func ensureUp() async throws -> String {
        ...
        guard await tearDown() else {
            throw HerdrError.tunnelFailed("timed out stopping owned SSH proxies")
        }

        let platform = try await probeRemotePlatform()
        // A cancelled probe may resume after a replacement owns this shared path.
    ...
    @discardableResult
    public func tearDown() async -> Bool {
        process?.terminate()
        process = nil
        let bridge = apiBridge
        resetErrorCapture()
        if bridge == nil, let localSocketPath {
            try? FileManager.default.removeItem(atPath: localSocketPath)
        }
        localSocketPath = nil
        let stopped = await bridge?.stopAndWait() ?? true
        if stopped, apiBridge === bridge { apiBridge = nil }
        return stopped
    }
    ...
    public func forwardingFailure() async -> String? {
        ...
        if let apiBridge { return apiBridge.lastError }
    ...
    public func diagnoseSilentForward() async -> HerdrError? {
        ...
        if case .windows = try? await probeRemotePlatform() {
            // Let the stderr pump finish after a short-lived SSH exec failure.
            try? await Task.sleep(nanoseconds: 150_000_000)
            if let reason = apiBridge?.lastError { return .tunnelFailed(reason) }
            return .tunnelFailed(
