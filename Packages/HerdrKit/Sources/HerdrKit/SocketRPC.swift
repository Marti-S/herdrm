...
public struct SocketRPC: Sendable {
    ...
    static func readLine(fd: Int32, timeoutSeconds: Int32?, buffer: inout Data) throws -> Data? {
        ...
        if setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &tv, socklen_t(MemoryLayout<timeval>.size)) != 0 {
            // A peer that replied and hung up refuses every setsockopt from
            // then on: XNU's `sosetopt` returns EINVAL once the socket is done
            // both ways (SS_CANTRCVMORE && SS_CANTSENDMORE), which is where a
            // closed peer puts us. herdr closes each request connection right
            // after writing its reply, so losing that race is routine — and the
            // reply is already sitting in the receive buffer. Read it instead
            // of reporting a connection failure. The unset timeout is harmless
            // too: SS_CANTRCVMORE means read() drains the buffer and then
            // returns EOF, so it cannot block. Any other errno (EBADF,
            // ENOTSOCK) is a genuine descriptor bug and still throws.
            let code = errno
            guard code == EINVAL else {
                throw HerdrError.connectionFailed("setsockopt(SO_RCVTIMEO): \(String(cString: strerror(code)))")
            }
        }
