...
final class FleetStore: ObservableObject {
    ...
    /// Compatibility projections keep feature call sites concise while navigation
    /// state has one explicit owner. Fleet and terminal lifetime state remains here.
    var deviceFilter: UUID? {
        get { navigation.deviceFilter }
        set { navigation.deviceFilter = newValue }
    }
    @Published var sessions: [UUID: DeviceSessionState] = [:]
    var selectedSpace: SpaceRef? {
        get { navigation.selectedSpace }
        set { navigation.selectedSpace = newValue }
    }
    var selectedPane: PaneRef? {
        get { navigation.selectedPane }
        set {
            let oldValue = navigation.selectedPane
            navigation.selectedPane = newValue
    ...
    var showAddDevice: Bool {
        get { navigation.showAddDevice }
        set { navigation.showAddDevice = newValue }
    }
    var showNewAgent: Bool {
        get { navigation.showNewAgent }
        set { navigation.showNewAgent = newValue }
    }
    var showNewTerminal: Bool {
        get { navigation.showNewTerminal }
        set { navigation.showNewTerminal = newValue }
    }
    var showNewSpace: Bool {
        get { navigation.showNewSpace }
        set { navigation.showNewSpace = newValue }
    }
    var showSearch: Bool {
        get { navigation.showSearch }
        set { navigation.showSearch = newValue }
    }
    var isFileManagerActive: Bool {
        get { navigation.isFileManagerActive }
        set { navigation.isFileManagerActive = newValue }
    }
    ...
    var pendingSplitAgentFocus: Bool {
        get { navigation.pendingSplitAgentFocus }
        set { navigation.pendingSplitAgentFocus = newValue }
    }
    ...
    @Published var splitRatio: Double =
        UserDefaults.standard.object(forKey: FleetStore.splitRatioKey) as? Double ?? 0.5
    {
        didSet {
            UserDefaults.standard.set(splitRatio, forKey: FleetStore.splitRatioKey)
    ...
    var selectedShellID: UUID? {
        get { navigation.selectedShellID }
        set { navigation.selectedShellID = newValue }
    }
    /// In-window device panel (NSPopover crashes in ViewBridge on macOS 26+ betas).
    var showDevicePanel: Bool {
        get { navigation.showDevicePanel }
        set { navigation.showDevicePanel = newValue }
    }
    var deviceToEdit: Device? {
        get { navigation.deviceToEdit }
        set { navigation.deviceToEdit = newValue }
    }
    var sshAuthenticationRequest: SSHAuthenticationRequest? {
        get { navigation.sshAuthenticationRequest }
        set { navigation.sshAuthenticationRequest = newValue }
    }
    var spaceToRename: SpaceEntry? {
        get { navigation.spaceToRename }
        set { navigation.spaceToRename = newValue }
    }
    var agentToRename: AgentEntry? {
        get { navigation.agentToRename }
        set { navigation.agentToRename = newValue }
    }
    var terminalToRename: TerminalEntry? {
        get { navigation.terminalToRename }
        set { navigation.terminalToRename = newValue }
    }
    /// Transient action failures: shown as an alert, never by tearing down sessions.
    var actionError: String? {
        get { navigation.actionError }
        set { navigation.actionError = newValue }
    }
    ...
    var closeRequest: CloseRequest? {
        get { navigation.closeRequest }
        set { navigation.closeRequest = newValue }
    }
