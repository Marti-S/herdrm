...
final class FleetBridgeServer: ObservableObject {
    ...
    private let snapshotCache = FleetBridgeSnapshotCache()
    private var token = ""
    ...
    func start(model: FleetStore) {
        ...
        do {
            try snapshotCache.initialize(devices: model.devices) {
                deviceSnapshot(device: $0, model: model)
            }
        } catch {
    ...
    var currentRevision: UInt64 { snapshotCache.revision }
    ...
    func snapshot() throws -> FleetSnapshot {
        ...
        return try snapshotCache.refresh(devices: model.devices) {
            deviceSnapshot(device: $0, model: model)
        }
    }
    ...
    func encodedSnapshot() throws -> Data {
        _ = try snapshot()
        return try snapshotCache.encodedSnapshot()
    }
    ...
    private func schedulePublish(_ change: FleetStateChange) {
        ...
        case .device(let deviceID):
            snapshotCache.invalidate(deviceID: deviceID)
        case .topology:
            snapshotCache.invalidateTopology()
        }
    ...
    private func publishPendingFleetChanges() {
        do {
            _ = try snapshot()
            guard let encodedSnapshot = try snapshotCache.pendingBroadcast(
                enabled: configuration.enabled,
                hasSubscribers: !subscriptions.isEmpty
            ) else { return }
            ...
            fleetBridgeLog.debug(
                "published fleet revision \(self.currentRevision) (\(encodedSnapshot.count) bytes) to \(self.subscriptions.count) subscribers"
            )
        } catch {
            fleetBridgeLog.error(
    ...
    private func deviceSnapshot(device: Device, model: FleetStore) -> FleetDeviceSnapshot {
    ...
    func deviceSessionSnapshot(device: Device, model: FleetStore) -> SessionSnapshot {
        let state = model.session(device.id)
        let version: String?
