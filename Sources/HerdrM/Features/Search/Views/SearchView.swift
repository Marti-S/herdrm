...
struct SearchSheet: View {
    ...
    enum Result: Identifiable {
        case agent(FleetStore.AgentEntry)
        case terminal(FleetStore.TerminalEntry)
        case space(FleetStore.SpaceEntry)
    ...
    private var results: [Result] {
        ...
        let spaces = model.devices.flatMap { device in
            model.session(device.id).workspaces.map { FleetStore.SpaceEntry(device: device, workspace: $0) }
        }.filter { entry in
            q.isEmpty
    ...
    private func searchRank(_ entry: FleetStore.AgentEntry) -> Int {
