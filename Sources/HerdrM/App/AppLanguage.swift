...
enum AppLanguage: String, CaseIterable, Identifiable {
    ...
    static func relaunch(
        spawnHelper: () -> pid_t? = spawnRelaunchHelper,
        terminate: @MainActor () -> Void = { NSApp.terminate(nil) },
        reportFailure: (@MainActor () -> Void)? = nil
    ) {
        ...
        // Never start a competing Fleet bridge owner while this process is alive.
        // If the detached waiter cannot launch, leave the app running and explain
        // how to apply the already-saved language choice manually.
        let alert = NSAlert()
        alert.messageText = String(localized: "Relaunch")
        alert.informativeText = String(localized: "Quit and reopen herdrm so the new language takes effect.")
        alert.runModal()
    }
